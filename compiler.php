...
class Compiler
{
     ...
     */
    private function tokenizer(): array
    {
        $input = $this->input . PHP_EOL;
    ...
    private function parser(array $tokens): Node
    {
        // 我们声明一个 `pc` 变量作为指针。
        $this->pc = 0;
        $this->pt = $tokens;
    ...
    private function walk(): Node
    {
        ...
        if ($token->kind == "paren" && $token->value == "(") {
            ...
            $n = new Node();
            $n->kind = "CallExpression";
            $n->name = $token->value;
            $n->params = [];

            // 我们再次自增 `pc` 变量，跳过当前的 token
            /** @var Node $token */
            $token = $this->pt[++$this->pc];
    ...
    private function traverser(node $a, array $v)
    {
        // 我们对 AST 调用 `traverseNode`，开始遍历。注意 AST 并没有父结点。
        $this->traverseNode($a, new Node(), $v);
    }

    // `traverseNode` 函数接受一个 `node` 和它的父结点 `parent` 作为参数，这个结点会被
    // 传入到 visitor 中相应的处理函数那里。
    private function traverseNode(node $n, node $p, array $v)
    {
        ...
        switch ($n->kind) {
            ...
            case "Program":
                $this->traverseArray($n->body, $n, $v);
                break;
            // 下面我们对 `CallExpressions` 做同样的事情，遍历它的 `params`。
            case "CallExpression":
                $this->traverseArray($n->params, $n, $v);
                break;
            // 如果是 `NumberLiterals`，那么就没有任何子结点了，所以我们直接 break
            case "NumberLiteral":
                break;
            // 如果我们不能识别当前的结点，那么就抛出一个错误。
            default:
                exit(1);
        }
    }
    ...
    private function transformer(Node $a): Node
    {
        ...
        $this->traverser($a, [
            ...
            "NumberLiteral" => function (Node $n, Node $p) {
                // 我们创建一个新结点，名字叫 `NumberLiteral`，并把它放入父结点的 context 中。
                $tmp = new Node();
                $tmp->kind = "NumberLiteral";
                $tmp->value = $n->value;
                $p->context[] = $tmp;
            },
            // 下一个，`CallExpressions`。
            "CallExpression" => function (Node $n, Node $p) {
                // 我们创建一个 `CallExpression` 结点，里面有一个嵌套的 `Identifier`。
                $tmp = new Node();
                $tmp->kind = "CallExpression";
                $tmp->callee = new Node();
                $tmp->callee->kind = "Identifier";
                $tmp->callee->name = $n->name;
                $tmp->arguments = [];
                // 下面我们在原来的 `CallExpression` 结点上定义一个新的 context，它是 expression
                // 中 arguments 这个数组的引用，我们可以向其中放入参数。
                $n->context = &$tmp->arguments;
    ...
    private function traverseArray(array $a, node $p, array $v)
    {
        foreach ($a as $child) {
            $this->traverseNode($child, $p, $v);
        }
    }
     ...
     */
    private function codeGenerator(Node $n): string
    {
        // 对于不同 `type` 的结点分开处理。
        switch ($n->kind) {
    ...
    public function compile()
    {
        ...
        $nast = $this->transformer($ast);
        // 然后返回输出!
        return $this->codeGenerator($nast);
    }
}
 ...
 * ============================================================================
 */
$c = new Compiler("(add 10 (subtract 10 6))");
var_dump($c->compile());
